...
const FLUSH_LOOKUP = new Map();
const UNSUITED_LOOKUP = new Map();
...
function multiples() {
  ...
  for (const pairrank of backwardsRanks) {
    ...
    for (const k of kgen) {
      ...
      const k2 = k[1];
      const k3 = k[2];
      const product = PRIMES[pairrank]**2 * PRIMES[k1] *
          PRIMES[k2] * PRIMES[k3];
      UNSUITED_LOOKUP.set(product, rank);
      rank += 1;
    }
...
function* getLexographicallyNextBitSequence(bits) {
  let t = (bits | (bits - 1)) + 1;
...
exports.FLUT = FLUSH_LOOKUP;
exports.ULUT = UNSUITED_LOOKUP;
